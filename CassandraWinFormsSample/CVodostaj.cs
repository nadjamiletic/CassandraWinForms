...
{
    public partial class CVodostaj : UserControl
    {

        public List<StanjeStanica> stanje;
        public CVodostaj()
        {
            InitializeComponent();
            cbRekaPopuni();
            //PopuniVodostaj("s", "r", "m");
        }

        public void PopuniVodostaj(string stanicaid, string rekaid, string mesec,int dan)
        {
            ...
            foreach (CassandraDataLayer.QueryEntities.StanjeStanica rek in CassandraDataLayer.DataProvider.GetPodaci(stanicaid, rekaid, mesec,dan))
            {
                ...
                dataGridView1.Rows[n].Cells[1].Value = Convert.ToString(rek.vodostaj);
        ...
        private void cbMesec_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //stanje = CassandraDataLayer.DataProvider.GetPodaci(cbStanica.Text,cbReka.Text,cbMesec.Text,int.Parse(cmbDan.Text));
        }

        private void button1_Click(object sender, EventArgs e)
        {
            VodostajForma v = new VodostajForma();
            v.VodostajGrafik(CassandraDataLayer.DataProvider.GetPodaci(cbStanica.Text, cbReka.Text, cbMesec.Text, int.Parse(cmbDan.Text)));
            v.Show();
        }

        private void cmbDan_SelectedIndexChanged(object sender, EventArgs e)
        {
            PopuniVodostaj(cbStanica.Text, cbReka.Text, cbMesec.Text,int.Parse(cmbDan.Text));
        }
    }
}
